---
title: "공개형 Base Model에서 Chat Model까지 — Llama 2: Open Foundation and Fine-Tuned Chat Models"
math: true
---
...
# 공개형 Base Model에서 Chat Model까지 — Llama 2: Open Foundation and Fine-Tuned Chat Models

이 글은 Llama 2 보고서를 단순한 성능표 모음이 아니라, **공개형 foundation model과 대화형 chat model을 함께 설명한 시스템 문서**로 읽게 해 주는 해설이다. 가장 먼저 구분할 것은 사전학습 기준선으로서의 **Llama 2 base model**과, 정렬을 거친 **Llama 2-Chat**이다. 전자는 공개형 LLM의 기반 성능을, 후자는 SFT·RLHF·안전성 튜닝을 거친 실사용 대화 모델의 성격을 보여 준다. 처음 읽는다면 아래 3분 요약 표와 Table 1, Section 3, Section 4를 먼저 보라.

## 핵심 요약
- **푸는 문제:** 공개 가능한 base model을 얼마나 강하게 만들 수 있고, 그 위에 chat alignment를 얹으면 실제 대화 품질이 얼마나 달라지는가.
- **핵심 아이디어:** 사전학습 스케일 업, 데이터 정제, 컨텍스트 확장, GQA 같은 base model 개선과, SFT / preference learning / iterative RLHF / safety tuning을 하나의 시스템으로 묶어 제시한다.
- **주요 결과:** 7B~70B 공개형 모델군을 제시했고, Llama 2-Chat은 당시 공개형 대화 모델 대비 강한 성능과 안전성 개선을 보였다.
- **왜 중요한가:** 이 문서는 “좋은 foundation model”과 “실제로 쓰기 좋은 chat model”이 어떻게 다른지, 그리고 그 차이를 만들기 위해 어떤 정렬 절차가 필요한지를 함께 보여 준다.
- **한계 / 주의점:** 사람 평가와 안전성 평가는 설정과 프롬프트 분포의 영향을 크게 받으며, 공개형 모델 비교 결과도 평가 설계에 따라 달라질 수 있다.

## 3분 요약
| 구분 | Llama 2 base | Llama 2-Chat |
| --- | --- | --- |
| 목적 | 공개형 foundation model 기준선 | 대화형 응답과 정렬 |
| 핵심 학습 | pretraining | SFT + preference data + RLHF + safety tuning |
| 읽을 때 볼 것 | 데이터 / 토큰 / 컨텍스트 / GQA | helpfulness / safety / GAtt / 사람 평가 |
| 대표 절 | Section 2 | Section 3, Section 4 |
| 오해하면 안 되는 점 | base가 곧 chat은 아니다 | chat 성능은 정렬 운영까지 포함한 결과다 |

## 먼저 읽을 포인트
- 이 글의 핵심은 “Llama 2가 얼마나 큰가”보다 **base와 chat의 역할이 다르다**는 점이다.
- Section 2는 모델 자체의 기반 능력을, Section 3은 정렬 파이프라인을, Section 4는 안전성 운영을 보여 준다.
- 부록은 길지만, Table 26 / Figure 26 / Figure 28 / Table 30 / Figure 30은 실제 운영상의 메시지가 강하므로 우선순위를 높여라.
...
### 해설
이 초록은 모델 가중치 공개 자체보다도, **정렬(alignment) 레시피의 공개**를 더 강하게 시사한다. 실제로 본문은 사전학습 세팅, SFT, RLHF, 안전 튜닝, 레드팀, 평가 방법론을 모두 기술한다. 따라서 이 논문은 단순한 성능 보고서라기보다 **제품 수준 대화형 LLM을 구성하는 절차적 보고서**에 가깝다.
...
### 해설
도입부는 Llama 2-Chat의 성능을 바로 결론으로 밀어붙이기보다, “왜 공개형 챗 모델이 어려운가”를 구조적으로 설명한다. 이 논문이 실제로 가치 있는 지점은 베이스 모델과 챗 모델의 성능 차이 그 자체보다, **그 차이를 만드는 비공개 정렬 절차를 공개 가능한 수준으로 문서화했다는 점**에 있다.
...
### 해설
여기서 중요한 점은 “더 많은 데이터”만이 아니라 **어떤 데이터를 더 많이 보게 했는가**이다. factual source를 업샘플링한다는 말은 지식 정확도와 환각 억제를 위한 데이터 분포 설계를 의미한다.
...
| Family | Model sizes | Pretraining tokens | Context length | GQA |
| --- | --- | --- | --- | --- |
| Llama 1 | 7B / 13B / 33B / 65B | 1.0T | 2k | 사용 안 함 |
| Llama 2 | 7B / 13B / 34B / 70B | 2.0T | 4k | 34B / 70B에 사용 |

이 표는 Llama 2의 개선을 “모델이 더 커졌다”는 한 줄로 요약하면 안 된다는 점을 보여 준다. 데이터 정제, 학습 토큰 수, 컨텍스트 길이, attention 설계(GQA) 같은 여러 변경이 함께 작동해 base model 성능이 올라갔다고 읽는 것이 정확하다.
...
### 해설
2k에서 4k로의 컨텍스트 확장은 긴 문서 이해, 긴 대화 히스토리 유지, 장문 요약에 중요한 변화를 의미한다. 반면 GQA는 그러한 확장이 추론 비용을 지나치게 키우지 않도록 하는 쪽의 설계다.
...
### 해설
이 섹션의 핵심은 “Llama 2가 공개형 베이스 모델 생태계에서는 강한 위치를 점하지만, 제품형 또는 최상위 폐쇄형 모델과는 아직 차이가 남는다”는 점이다. 이는 뒤의 fine-tuning과 safety 섹션에서 왜 정렬 절차가 중요한지 다시 설명해 준다.
...
### 해설
이 설계는 “사용자 입력을 예측하는 모델”이 아니라 “사용자 입력 다음에 어떤 assistant 응답이 와야 하는지 학습하는 모델”을 만들기 위한 자연스러운 선택이다. 또한 본 절의 핵심 메시지는 SFT 데이터의 절대 규모보다 **데이터의 정제 정도와 대화형 적합성**이 더 중요할 수 있다는 점이다.
...
### 해설
이 절은 RLHF를 단일 알고리즘으로 이해하면 부족하다는 점을 보여준다. 저자들의 실제 시스템은 reward model 업데이트, rejection sampling, PPO, 안전 보상 결합, 분포 이동 대응, 과거 샘플 재사용 등 여러 요소가 결합된 **반복적 생산 공정**에 가깝다.
...
여기서 중요한 것은 GAtt가 새로운 attention layer가 아니라 **학습 데이터 구성 방식**이라는 점이다. 즉 첫 턴의 지시를 이후 사용자 메시지들에 “유령처럼” 다시 섞어 넣어, 모델이 멀티턴 대화에서도 초기 지시를 오래 유지하도록 학습시키는 방법으로 이해하면 된다.
...
### 해설
GAtt는 attention mechanism 자체를 수정한 기법이 아니라, **attention이 그러한 동작을 하도록 유도하는 데이터 구성 방식**이다. 따라서 이 기법의 핵심은 모델 구조보다 학습 입력 포맷에 있다.
...
### 해설
이 절은 Llama 2-Chat의 정렬 성능을 주장하면서도, 동시에 모델 기반 평가와 사람 평가 모두가 불완전함을 인정한다. 특히 reward model을 최적화 대상으로 사용할 때 인간 선호와의 괴리가 생길 수 있다는 인식은, 본 논문의 정렬 실험이 단순한 자동 최적화가 아니라 지속적인 교차 검증을 필요로 하는 과정임을 보여준다.
...
### 해설
이 절은 “사전학습 데이터가 안전해야만 안전한 모델이 된다”는 단순한 도식이 충분하지 않음을 보여준다. 저자들은 의도적으로 사전학습 단계의 과잉 정제를 피하고, downstream alignment와 evaluation에 더 많은 책임을 부여한다. 따라서 Llama 2의 안전성은 사전학습 데이터 자체보다, 이후의 정렬·레드팀·평가 체계와 함께 이해해야 한다.
...
이 절은 안전성을 별도의 분류기 후처리 문제로 다루지 않고, **더 안전한 응답 예시를 다시 학습 데이터로 증류하는 방식**으로 접근한다는 점이 중요하다. 다시 말해 safety preprompt는 추론 시 장식이 아니라, 안전 행동을 모델 파라미터 안으로 옮겨 넣기 위한 교사 신호 역할을 한다.
...
### 해설
이 절의 요지는 “안전은 평균을 올리는 문제가 아니라 tail을 줄이는 문제”라는 데 있다. Safety RLHF와 context distillation은 특히 adversarial prompt에서의 실패를 줄이는 데 초점을 맞춘다. 동시에 false refusal이 증가할 수 있기 때문에, 안전과 유용성의 긴장은 끝까지 남는다.
...
### 해설
이 절은 레드팀을 단순한 “해킹 시연”이 아니라, annotation과 fine-tuning으로 다시 유입되는 **학습 데이터 생성 장치**로 다룬다. 이는 안전 개선이 정적 규칙이 아니라 동적 업데이트 루프임을 잘 보여준다.
...
### 해설
이 평가는 “안전 모델이 짧게만 답해서 위반을 피한 것인지, 실제로 안전하면서도 유용한지”를 구분하려는 시도를 포함한다. 그래서 violation percentage만이 아니라 mean rating, false refusal, multi-turn 취약성, TruthfulQA, ToxiGen, BOLD를 함께 보는 구조가 된다.

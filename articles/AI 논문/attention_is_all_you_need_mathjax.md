...
# Transformer의 시작 — Attention Is All You Need

이 글은 Transformer가 왜 현대 LLM의 출발점처럼 읽히는지 빠르게 잡아 주는 해설이다. 이 논문의 핵심은 RNN이나 CNN 없이 **self-attention만으로** 시퀀스 변환 모델을 구성할 수 있음을 보였다는 데 있다. 그래서 중요한 것은 단순한 성능 수치보다도, **순차 계산 중심 구조를 병렬 행렬 연산 중심 구조로 바꿨다**는 아키텍처 전환이다. 처음 읽는다면 3장 Model Architecture, 4장 Why Self-Attention, 6장 Results를 중심으로 보면 된다.

## 핵심 요약
- **푸는 문제:** RNN 기반 seq2seq가 가진 순차 계산 병목을 줄이면서도, 번역 같은 시퀀스 변환 과제에서 높은 품질을 낼 수 있는가.
- **핵심 아이디어:** recurrence와 convolution을 걷어내고, **scaled dot-product attention + multi-head attention + positional encoding + FFN**을 하나의 표준 구조로 묶었다.
- **주요 결과:** WMT14 EN-DE/EN-FR 번역에서 강한 성능을 보였고, 무엇보다 **병렬화가 쉬워 학습 효율이 크게 좋아졌다**.
- **왜 중요한가:** 이후의 대형 언어모델은 세부 변형은 달라도, 큰 뼈대에서는 이 논문이 정리한 Transformer 설계를 계승한다.
- **한계 / 주의점:** self-attention은 시퀀스 길이에 대해 \(O(n^2)\) 비용이 들기 때문에, 긴 문맥에서는 계산량 문제가 남는다.

## 먼저 읽을 포인트
- 이 논문은 “attention이 유용하다”를 말하는 글이 아니라, **attention만으로도 표준 모델을 세울 수 있다**고 주장하는 글이다.
- 3장은 구조를, 4장은 왜 self-attention이 유리한지를, 6장은 그 주장이 실험으로 어떻게 뒷받침되는지를 보여 준다.
- 처음 읽을 때는 세부 하이퍼파라미터보다 **병렬화 / path length / 계산복잡도** 세 키워드를 먼저 잡아라.

원문 논문: Vaswani et al., 2017.
...
대형 시스템 논문에서 ‘저자 기여’는 구현/재현 관점에서 코드 출처와 핵심 아이디어 주도자를 파악하는 데 도움됩니다.
...
‘recurrence/conv 배제’는 RNN/CNN 블록을 쓰지 않는다는 뜻이며, 대신 선형층/FFN/LayerNorm/잔차 연결은 존재합니다.
...
여기서 ‘해상도(resolution)’는 평균화로 정보가 뭉개질 수 있다는 직관이며, 논문은 이를 multi-head로 상쇄한다고만 말합니다(정량 정의는 없음).
...
이 식은 Transformer의 핵심 연산이 실제로 어떻게 이루어지는지를 가장 짧게 보여 준다. 여기서 봐야 할 것은 “어떤 토큰이 다른 토큰을 얼마나 참고할지”가 **Q와 K의 유사도**로 정해지고, 실제 전달되는 정보는 **V의 가중합**으로 나온다는 점이다.

Attention(Q,K,V) = softmax((QK^T)/sqrt(d_k)) V  (식 (1))

해석은 단순하다. 어떤 query가 특정 key와 더 잘 맞을수록 그 위치의 value를 더 많이 가져온다. \(\sqrt{d_k}\)로 나누는 이유는 차원이 커질수록 내적값이 과하게 커져 softmax가 너무 날카로워지는 것을 완화하기 위해서다.
...
이 표는 왜 Transformer가 “더 빠른 학습” 이야기와 함께 등장하는지를 압축해서 보여 준다. self-attention은 \(O(n^2)\) 비용이라는 약점이 있지만, **순차 연산 수와 최대 경로 길이** 측면에서는 RNN보다 훨씬 유리하다.

...
이 논문은 단순히 “어텐션이 잘 된다”를 보여 준 문서가 아니다. 더 정확히 말하면, **시퀀스 모델링의 핵심 병목을 순환(recurrence)에서 병렬화 가능한 self-attention으로 재정의한 아키텍처 전환점**이다. 저자들의 공헌은 세 가지 층위에서 읽어야 한다. 첫째, **표현 관점**에서는 query-key-value 구조와 multi-head 분해를 통해 서로 다른 관계 유형을 병렬로 포착하는 방식을 제안했다. 둘째, **계산 관점**에서는 RNN의 순차 의존성을 제거해 학습을 크게 병렬화했다. 셋째, **실험 관점**에서는 WMT 번역과 구문 분석에서 강한 품질을 더 낮은 학습 비용으로 입증했다.

이 논문을 현대 관점에서 읽을 때 중요한 포인트는 “Transformer = attention”이라는 단순 도식에 머무르지 않는 것이다. 실제 성능은 **잔차 연결, layer normalization, position-wise feed-forward, label smoothing, warmup schedule, shared embedding/softmax 설계**가 함께 맞물려 나온다. 즉, self-attention은 중심 기술이지만, 논문의 설계 완성도는 최적화와 regularization까지 포함한 **시스템 수준 조합**에 있다.

연구적으로는 두 가지를 특히 주의해 읽는 것이 좋다. 첫째, 저자들이 말한 복잡도 비교는 **시퀀스 길이 \(n\)** 와 표현 차원 \(d\) 사이의 상대 크기에 의존하며, 긴 문맥에서는 \(O(n^2)\) 비용이 다시 병목이 된다. 둘째, positional encoding은 순서를 “복구”하는 장치이지, self-attention의 한계를 완전히 없애는 마법이 아니다. 따라서 이 논문은 곧바로 long-context와 sparse attention 연구로 이어지는 출발점이기도 하다.

독해할 때는 다음 질문을 계속 붙들면 좋다.  
1. 왜 dot-product attention에 \(1/\sqrt{d_k}\) 스케일링이 필요한가?  
2. multi-head는 단지 파라미터 수를 늘리는 장치인가, 아니면 표현 공간 분할의 이점이 있는가?  
3. self-attention의 짧은 path length가 실제로 어떤 종류의 의존성 학습을 쉽게 만드는가?  
4. 이 논문의 승리는 attention 하나의 승리인가, 아니면 전체 훈련 recipe의 승리인가?
...
> - 권장 캡션: `Table 2: BLEU 및 Training Cost(FLOPs)`  
> - 편집 원칙: 도판 자체는 원문 이미지를 사용하고, 본문에서는 **이 도판이 무엇을 보여 주는지**만 해설합니다.
...
> - 권장 캡션: `Table 4: WSJ Section 23 F1`  
> - 편집 원칙: 도판 자체는 원문 이미지를 사용하고, 본문에서는 **이 도판이 무엇을 보여 주는지**만 해설합니다.
